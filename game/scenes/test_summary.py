...
class TestSummaryScene(Scene):
    ...
    def _draw_title(self, surface: pygame.Surface) -> None:
        ...
        heading_key = "test_summary.title_time_up" if self.time_up else "test_summary.title_finished"
        heading_text = self.tr(
            heading_key,
            default="Tijd is op!" if self.time_up else "Test afgerond!",
        )
        title = self.title_font.render(heading_text, True, settings.COLOR_TEXT_PRIMARY)
        surface.blit(title, title.get_rect(topleft=(back_right, margin - 30)))

        subtitle_text = self.tr(
            "test_summary.subtitle",
            default="Goed gedaan {name}! Snelheid: {speed}",
            name=self.result.profile_name,
    ...
    def _draw_stats(self, surface: pygame.Surface) -> None:
        ...
        rows = [
            (
                self.stat_font,
                self.tr(
                    "test_summary.stats.correct",
                    default=self.tr(
                        "common.stats.correct",
                        default="Goed: {count}",
                        count=self.result.correct,
                    ),
                    count=self.result.correct,
                ),
            ...
            (
                self.stat_font,
                self.tr(
                    "test_summary.stats.incorrect",
                    default=self.tr(
                        "common.stats.incorrect",
                        default="Fout: {count}",
                        count=self.result.incorrect,
                    ),
                    count=self.result.incorrect,
                ),
            ...
            (
                self.stat_font,
                self.tr(
                    "test_summary.stats.accuracy",
                    default=self.tr(
                        "common.stats.accuracy",
                        default="Nauwkeurigheid: {percentage}%",
                        percentage=f"{self.result.accuracy * 100:.0f}",
                    ),
                    percentage=f"{self.result.accuracy * 100:.0f}",
                ),
            ...
            (
                self.helper_font,
                self.tr(
                    "test_summary.stats.answered",
                    default=self.tr(
                        "common.stats.answered",
                        default="Beantwoord: {answered} van {total}",
                        answered=self.result.answered,
                        total=self.result.question_count,
                    ),
                    answered=self.result.answered,
                    total=self.result.question_count,
                ),
            ...
            (
                self.helper_font,
                self.tr(
                    "test_summary.stats.time_used",
                    default="Gebruikte tijd: {used}s van {limit}s",
                    used=f"{self.result.elapsed_seconds:.0f}",
                    limit=self.result.time_limit_seconds,
                ),
                settings.COLOR_TEXT_DIM,
            ),
            (
                self.helper_font,
                self.tr(
                    "test_summary.stats.coins_change",
                    default="Munten: {delta:+d} (totaal {total})",
                    delta=self.coin_delta,
                    total=self.total_coins,
                ),
                settings.COLOR_TEXT_DIM,
            ),
        ]
        rendered = [font.render(text, True, colour) for font, text, colour in rows]
        max_width = max(line.get_width() for line in rendered)
        card_width = max(420, max_width + 64)
        card_height = 40 + sum(line.get_height() + 16 for line in rendered)
    ...
    def _draw_history(self, surface: pygame.Surface) -> None:
        ...
        header_text = self.tr(
            "test_summary.history.title",
            default="Foutjes om van te leren",
        )
        ...
        if not mistakes:
            empty_text = self.tr(
                "test_summary.history.empty",
                default="Geen foutjes! Geweldig!",
            )
            content_lines = [self.helper_font.render(empty_text, True, settings.COLOR_SELECTION)]
        else:
            content_lines = [
                self.helper_font.render(
                    self.tr(
                        "test_summary.history.item",
                        default="{left} x {right} = {answer} (jij zei {given})",
                        left=question.left,
    ...
    def _draw_buttons(self, surface: pygame.Surface) -> None:
        ...
        retry_label = self.tr(
            "test_summary.actions.retry",
            default=self.tr("common.play_again", default="Nog een keer!"),
        )
        retry_button = Button(
            retry_rect,
            retry_label,
        ...
        menu_label = self.tr(
            "test_summary.actions.menu",
            default=self.tr("common.return_menu", default="Terug naar menu"),
        )
        menu_button = Button(
            menu_rect,
            menu_label,
    ...
    def _generate_suggestion(self) -> str:
        ...
        if all(value.get("questions", 0) == value.get("correct", 0) for value in self.table_stats.values()):
            return self.tr(
                "test_summary.suggestion.perfect",
                default="Fantastisch! Alle tafels gingen vlekkeloos. Kies gerust een nieuw avontuur.",
            )
        ...
        if tricky:
            ...
            return self.tr(
                "test_summary.suggestion.focus",
                default=(
                    "Neem de tafel van {table} nog even door. Er gingen {incorrect} sommen mis en het kost gemiddeld {average}s. Een oefenrondje maakt je supersnel!"
                ),
                table=table,
                incorrect=incorrect,
        ...
        if slow:
            ...
            average = stats.get("total_time", 0.0) / max(stats.get("questions", 1), 1)
            return self.tr(
                "test_summary.suggestion.speed",
                default=(
                    "Je bent heel precies! Misschien kun je de tafel van {table} nog eens oefenen om nog sneller te worden. Nu kost een som gemiddeld {average}s."
                ),
                table=table,
                average=f"{average:.1f}",
            )
        ...
        return self.tr(
            "test_summary.suggestion.default",
            default="Blijf oefenen en daag jezelf uit met een mix van tafels voor nog meer snelheid.",
        )
